...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  out.center_2d.x = ((t.x / t.y) * fx) + half_w; 
  ...
  let min_pixel = half_w - vec2<f32>(radius);
  let max_pixel = half_h + vec2<f32>(radius);
  ...
  //     keys_buffer[global_key_idx].key = key;
  //     keys_buffer[global_key_idx].value = current_visible_gaussian_index; 
  //   }
  // }
