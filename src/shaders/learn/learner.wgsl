...
fn main(
  ...
  let dL_dopacity = f32(global_grads.grads[g_id].opacity)  / scale_f;
  let dL_dcolor   = vec3f(
    f32(global_grads.grads[g_id].color_r) / scale_f,
    f32(global_grads.grads[g_id].color_g) / scale_f,
    f32(global_grads.grads[g_id].color_b) / scale_f
  );
  let dL_dcenter_2d = vec2f(
    f32(global_grads.grads[g_id].center_x) / scale_f,
    f32(global_grads.grads[g_id].center_y) / scale_f
  );
  let dL_dcov_2d = vec3f(
    f32(global_grads.grads[g_id].cov_a) / scale_f,
    f32(global_grads.grads[g_id].cov_b) / scale_f,
    f32(global_grads.grads[g_id].cov_c) / scale_f
  );
  ...
  dL_dxyz_camera.x = dL_dcenter_2d.x * J_00;
  dL_dxyz_camera.y = dL_dcenter_2d.y * J_11;
  ...
  let dL_dSigma2D = mat3x3f(
    vec3<f32>(dL_dcov_2d.x,       dL_dcov_2d.y * 0.5, 0.0),
    vec3<f32>(dL_dcov_2d.y * 0.5, dL_dcov_2d.z,       0.0),
    vec3<f32>(0.0, 0.0, 0.0)
  );

  let J = mat3x3<f32>(
    vec3<f32>(J_00, 0.0,  J_02),
    vec3<f32>(0.0,  J_11, J_12),
    vec3<f32>(0.0,  0.0,  0.0)
  );
  let w_mat = mat3x3<f32>(
    camera.view[0].xyz,
    camera.view[1].xyz,
    camera.view[2].xyz
  );
  ...
  let T            = J * w_mat;
  let dL_dSigma3D  = transpose(T) * dL_dSigma2D * T;
  ...
  let current_log_scale    = log(max(current_linear_scale, vec3f(1e-8)));
  ...
  let new_log_scale  = current_log_scale - lr_scale
    * (adam_M[g_id].scale / bias_corr1)
    / (sqrt(adam_V[g_id].scale / bias_corr2) + eps);
  ...
  let new_rot = inputData[g_id].rotation
    - lr_rotation * (adam_M[g_id].rotation / bias_corr1)
    / (sqrt(adam_V[g_id].rotation / bias_corr2) + eps);
  ...
  let new_color = inputData[g_id].color.xyz
    - lr_color * (adam_M[g_id].color.xyz / bias_corr1)
    / (sqrt(adam_V[g_id].color.xyz / bias_corr2) + eps);
  inputData[g_id].color = vec4f(clamp(new_color, vec3f(0.0), vec3f(1.0)), 1.0);
  ...
  let new_logit_op = current_logit_op
    - lr_opacity * (adam_M[g_id].opacity / bias_corr1)
    / (sqrt(adam_V[g_id].opacity / bias_corr2) + eps);
  ...
  inputData[g_id].position -= lr_xyz
    * (adam_M[g_id].position / bias_corr1)
    / (sqrt(adam_V[g_id].position / bias_corr2) + eps);
}