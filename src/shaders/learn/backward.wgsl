...
fn isNan(val: f32) -> bool { return val != val; }
...
fn main(
  ...
  let tile_x  = global_id.x / tile_size;
  let tile_y  = global_id.y / tile_size;
  ...
  for (var i = i32(end_idx) - 1; i >= i32(start_idx); i -= 1) {
    ...
    let det = a * c - b * b;
    if (det < 1e-6 || isNan(det)) { continue; }
    ...
    if (power > 0.0 || isNan(power)) { continue; }
    ...
    let dL_da = -(inv_cov_00 * inv_cov_00 * dL_dinv_cov_00
                + inv_cov_00 * inv_cov_01 * dL_dinv_cov_01
    ...
    atomicAdd(&global_grads.grads[g_id].cov_a,     i32(round(dL_da           * scale)));
    atomicAdd(&global_grads.grads[g_id].cov_b,     i32(round(dL_db           * scale)));
    atomicAdd(&global_grads.grads[g_id].cov_c,     i32(round(dL_dc           * scale)));

    C_after = C_after * (1.0 - alpha) + data.color * alpha;
